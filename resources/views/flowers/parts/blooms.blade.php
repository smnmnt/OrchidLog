...
<li class="list-group-item mb-5"><a href="{{route('flowers.blooms.create', ['id' => $Unit->ID]) }}" class="link-primary">Добавить</a></li>
@if(isset($blooms) && sizeof($blooms))
    <table class="table table-striped table-bordered text-center align-middle">
        <thead>
        <tr>
            <th scope="col">#</th>
            <th scope="col" class="table_blooms_40pr">Дата начала</th>
            <th scope="col" class="table_blooms_40pr">Дата завершения</th>
            <th scope="col"></th>
        </tr>
        </thead>
        <tbody>
                            ...
                            <ul class="dropdown-menu" style="min-width: 0">
                                ...
                                <li>
                                    <form action="{{ route('flowers.blooms.destroy', ['id' => $bloom->ID]) }}"
                                          ...
                                          method="post"
                                          @if($bloom->BE)
                                              onsubmit="return confirm('Удалить цветение? {{date('d.m.Y', strtotime($bloom->BB))."-".date('d.m.Y', strtotime($bloom->BE)) }}');"
                                            @else
                                              onsubmit="return confirm('Удалить цветение? {{date('d.m.Y', strtotime($bloom->BB))."-"."Без закрытой даты" }}');"
                                          @endif
                                    >
                                        @csrf
                                        @method('DELETE')
