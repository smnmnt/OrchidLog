@extends('layouts.layout', ['title' => 'Добавить тест воды'])
...
@section('content')
	...
	<div class="container mt-4">
		<h2 class="mb-3">Добавить тест воды</h2>
		...
		<form method="POST" action="{{ route('aqua.test.store') }}" class="form-box">
			@csrf
			...
			<div class="row mb-3">
				...
				<input
					...
					name="tested_at"
					id="tested_at"
					value="{{ old('tested_at', now()->format('Y-m-d\TH:i')) }}"
					required
				>
				@error('tested_at')
			...
			<div class="row mb-3">
				...
				<select class="form-control @error('source_type') is-invalid @enderror" name="source_type" id="source_type" required>
					<option value="{{ \App\Models\AquaTests::SOURCE_TYPE_AQUARIUM }}" @selected(old('source_type', \App\Models\AquaTests::SOURCE_TYPE_AQUARIUM) === \App\Models\AquaTests::SOURCE_TYPE_AQUARIUM)>
						Аквариум
					</option>
					<option value="{{ \App\Models\AquaTests::SOURCE_TYPE_TAP }}" @selected(old('source_type') === \App\Models\AquaTests::SOURCE_TYPE_TAP)>
						Другой
					</option>
				</select>
				@error('source_type')
			...
			<div class="row mb-3" id="aquarium-field">
				...
				<select class="form-control @error('aquarium_id') is-invalid @enderror" name="aquarium_id" id="aquarium_id">
					...
					@foreach($aquariums as $aquarium)
						<option value="{{ $aquarium->id }}" @selected((string) old('aquarium_id') === (string) $aquarium->id)>
							{{ $aquarium->name }}
						</option>
					@endforeach
				</select>
			...
			<div class="row mb-3" id="source-name-field">
				...
				<input
					...
					name="source_name"
					id="source_name"
					value="{{ old('source_name', 'Водопровод') }}"
